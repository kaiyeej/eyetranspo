...
{
	global $mysqli_connect;
	$fetch = $mysqli_connect->query("SELECT * FROM tbl_transactions WHERE trip_id='$trip_id' AND `status`!='C' AND transaction_id=(SELECT MAX(transaction_id) FROM tbl_transactions)");
	$row = $fetch->fetch_array();
	return $row;
}
...
function getUserDestination($user_id, $trip_id)
{
	...
	$fetch = $mysqli_connect->query("SELECT destination FROM tbl_transactions WHERE user_id='$user_id' AND trip_id='$trip_id' AND `status`!='C' AND transaction_id=(SELECT MAX(transaction_id) FROM tbl_transactions)");
	$row = $fetch->fetch_array();
	if (empty($row[0])) {
		$location = 0;
	} else {
...
function getTransactionStatus($user_id)
{
	...
	$fetch = $mysqli_connect->query("SELECT `status` FROM tbl_transactions WHERE user_id='$user_id' AND `status`!='C' AND transaction_id=(SELECT MAX(transaction_id) FROM tbl_transactions)");
	$data = $fetch->fetch_array();
